...
Session::Options::Options(const Session::Options& other)
    : alpn(other.alpn),
      ...
      request_ocsp(other.request_ocsp),
      verify_hostname_identity(other.verify_hostname_identity),
      handshake_timeout(other.handshake_timeout),
      min_dh_size(other.min_dh_size),
      psk_callback_present(other.psk_callback_present),
      session_id_ctx(other.session_id_ctx),
      resume(other.resume),
      early_transport_params(other.early_transport_params),
      early_session_ticket(other.early_session_ticket) {}
...
BaseObjectPtr<Stream> Session::CreateStream(stream_id id) {
  ...
  USE(state->stream_ready_callback()->Call(
      env()->context(),
      object(),
      arraysize(argv),
      argv));
...
bool Session::HandshakeCompleted(const SocketAddress& remote_addr) {
  RemoteTransportParamsDebug transport_params(this);
  Debug(this, "Handshake completed with %s. %s", remote_addr, transport_params);
  RecordTimestamp(&SessionStats::handshake_completed_at);

  if (is_server()) {
    uint8_t token[kMaxTokenLen];
    size_t tokenlen = 0;
    if (!endpoint()->GenerateNewToken(token, remote_addr).To(&tokenlen)) {
...
void Session::PathValidation(
  ...
  Local<Value> argv[] = {
    ...
    SocketAddressWrap::Create(env(), SocketAddress(path->local.addr))->object(),
    SocketAddressWrap::Create(env(), SocketAddress(path->remote.addr))->object()
  };

  BaseObjectPtr<Session> ptr(this);

  USE(state->session_path_validation_callback()->Call(
      env()->context(),
      object(),
      arraysize(argv),
      argv));
...
void Session::UsePreferredAddress(const PreferredAddress::Address& address) {
  ...
  USE(state->session_use_preferred_address_callback()->Call(
      env()->context(),
      object(),
      arraysize(argv),
      argv));
...
int Session::OnHandshakeCompleted(ngtcp2_conn* conn, void* user_data) {
  ...
  return session->HandshakeCompleted(SocketAddress(path->remote.addr))
...
void OptionsObject::SetTLSOptions(const FunctionCallbackInfo<Value>& args) {
  ...
  if (UNLIKELY(options->SetOption(
      ...
      UNLIKELY(options->SetOption(
          obj,
          state->handshake_timeout_string(),
          &Session::Options::handshake_timeout).IsNothing()) ||
      UNLIKELY(options->SetOption(
          obj,
          state->min_dh_size_string(),
  ...
  options->options()->psk_callback_present =
      obj->Get(env->context(), state->pskcallback_string()).ToLocal(&val) &&
      val->IsFunction();
}
