...
void Session::CryptoContext::MaybeSetEarlySession(
    const std::shared_ptr<Options>& options) {
  // TODO(@jasnell): Reimplement session resume
...
void Session::PathValidation(
  ...
  Local<Value> argv[] = {
    ...
    SocketAddressBase::Create(
        env(),
        std::make_shared<SocketAddress>(path->local.addr))->object(),
    SocketAddressBase::Create(
        env(),
        std::make_shared<SocketAddress>(path->remote.addr))->object()
  };
...
void OptionsObject::New(const FunctionCallbackInfo<Value>& args) {
  ...
  CHECK_IMPLIES(  // IPv4 Preferred Address
      !args[7]->IsUndefined(),
      SocketAddressBase::HasInstance(env, args[7]));
  CHECK_IMPLIES(  // IPv6 Preferred Address
      !args[8]->IsUndefined(),
      SocketAddressBase::HasInstance(env, args[8]));
  ...
  if (!args[5]->IsUndefined()) {
    Local<Object> secure = args[5].As<Object>();
  ...
  if (!args[6]->IsUndefined()) {
    Local<Object> params = args[6].As<Object>();
  ...
  if (!args[7]->IsUndefined()) {
    ...
    ASSIGN_OR_RETURN_UNWRAP(&preferred_addr, args[7]);
  ...
  if (!args[8]->IsUndefined()) {
    SocketAddressBase* preferred_addr;
    ASSIGN_OR_RETURN_UNWRAP(&preferred_addr, args[8]);
