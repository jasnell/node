...
class QuicSocket final : public AsyncWrap,
                         public UDPListener,
 ...
 public:
  ...
  explicit QuicSocket(const QuicSocket& other) = delete;
  explicit QuicSocket(const QuicSocket&& other) = delete;
  QuicSocket& operator=(const QuicSocket& other) = delete;
  QuicSocket& operator=(const QuicSocket&& other) = delete;
...
#undef V
  ...
  const uint8_t* session_reset_secret() const { return reset_token_secret_; }

  // Serializes and transmits a RETRY packet for the specified
  // CIDs and address pair.
  bool SendRetry(
      const QuicCID& dcid,
      const QuicCID& scid,
      const SocketAddress& local_addr,
  ...
  // Serializes and transmits a Stateless Reset packet for the
  // specified CID and address pair.
  ...
  // Serializes and transmits a Version Negotiation packet for
  // the specified CID's and address pair.
  ...
  // Sends an immediate CONNECTION_CLOSE for the specified
  // CID's and address pair.
