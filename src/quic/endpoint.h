...
// An Endpoint encapsulates the UDP local port binding and is responsible.
// A single endpoint can act as both a QUIC client and server simultaneously.
class Endpoint final : public AsyncWrap,
                       public Packet::Listener {
 public:
  ...
  struct Options final : public MemoryRetainer {
    ...
    // sessions.
    uint8_t reset_token_secret[TOKEN_SECRET_LENGTH];

    // The secret used for generating new tokens.
    uint8_t token_secret[TOKEN_SECRET_LENGTH];
    ...
    // Generate the secret used for stateless resets.
    // This is called automatically by the Options constructor so there really
    // shouldn't be much reason to call it directly.
    void GenerateResetTokenSecret();

    // Generate the secret used for other tokens.
    // This is called automatically by the Options constructor so there really
    // shouldn't be much reason to call it directly.
    void GenerateTokenSecret();
  ...
  // Generate a new StatelessResetToken using the secret configured for
  // this endpoint.
  StatelessResetToken GenerateNewStatelessResetToken(
  ...
  BaseObjectPtr<Session> Connect(const SocketAddress& remote_address,
                                 ...
                                 BaseObjectPtr<SessionTicket> sessionTicket
                                     = BaseObjectPtr<SessionTicket>());

  // Returns the local address only if the endpoint has been bound. Before
  // the endpoint is bound, or after it is closed, this will return
  // std::nullopt.
...
#undef V
  ...
  public:
    ...
    // or std::nullopt if the UDP port is not currently bound.
    std::optional<SocketAddress> local_address() const;
