...
// Every QUIC Session has exactly one CryptoContext that maintains the state
...
class CryptoContext final : public MemoryRetainer {
 public:
  static inline const CryptoContext& From(const SSL* ssl);
  static inline CryptoContext& From(SSL* ssl);

  struct Options final : public MemoryRetainer {
    // The protocol identifier to be used by this Session.
    std::string alpn = QUIC_ALPN_H3;
  ...
  const std::string_view requested_alpn() const;
  const std::string_view requested_servername() const;
