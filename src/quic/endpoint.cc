...
inline bool IsShortHeader(quic_version version, const CID& cid) {
  return version == NGTCP2_PROTO_VER_MAX && !cid;
}
}  // namespace
...
void Endpoint::OnReceive(
  ...
  if (listener == nullptr) {
    bool is_short_header = IsShortHeader(pversion, scid);
...
Maybe<size_t> EndpointWrap::GenerateNewToken(
    uint8_t* token,
    const SocketAddress& remote_addr) {
  return inner_->GenerateNewToken(token, remote_addr);
}
...
void EndpointWrap::Close() {
  ...
  MakeCallback(state->endpoint_close_callback(), arraysize(argv), argv);
}
...
void EndpointWrap::OnError(Local<Value> error) {
  ...
  if (UNLIKELY(error.IsEmpty()))
    error = ERR_QUIC_INTERNAL_ERROR(env()->isolate(), "OnError failure");
