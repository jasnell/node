...
var b = Buffer.allocUnsafe(1024); // safe constructor
...
// Test Compare
{
  const b = Buffer.allocUnsafe(1).fill('a');
  const c = Buffer.allocUnsafe(1).fill('c');
  const d = Buffer.allocUnsafe(2).fill('aa');
...
// Test Equals
{
  const b = Buffer.allocUnsafe(5).fill('abcdf');
  const c = Buffer.allocUnsafe(5).fill('abcdf');
  const d = Buffer.allocUnsafe(5).fill('abcde');
  const e = Buffer.allocUnsafe(6).fill('abcdef');
...
assert.throws(
  function() { Buffer.allocUnsafe(1422561062959).toString('utf8'); });
...
// Must be zero-filled.
// This can be a bit unreliable because the allocated memory might
// actually already be filled with zeroes... the test is iterated
// multiple times to make it more reliable
{
  for (let n = 0; n < 50; n++) {
...
// Must be filled with 1's
{
  const buf = Buffer.alloc(10, 1);
  for (let i = 0; i < buf.length; i++)
    assert.equal(buf[i], 1);
}

// Must be filled with 'ABC', decoded from hex
{
  const check = [65, 66, 67];
  const buf = Buffer.alloc(9, '414243', 'hex');
  for (let i = 0; i < buf.length; i++)
    assert.equal(buf[i], check[i % 3]);
  assert.equal(buf.toString(), 'ABCABCABC');
}

// Throws due to invalid arguments
