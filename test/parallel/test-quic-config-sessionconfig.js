...
['abc', {}, [], null].forEach((handshakeTimeout) => {
  assert.throws(() => new SessionConfig({ secure: { handshakeTimeout } }), {
    code: 'ERR_INVALID_ARG_TYPE',
  });
});

['abc', {}, [], null].forEach((minDHSize) => {
  assert.throws(() => new SessionConfig({ secure: { minDHSize } }), {
    code: 'ERR_INVALID_ARG_TYPE',
  });
});
