...
/**
 * The Buffer(arg, encoding) construtor is "soft deprecated" ... that is,
 * it is deprecated in the documentation and should not be used moving
 * forward. Rather, developers should use one of the three new factory
 * APIs: Buffer.from(), Buffer.allocUnsafe() or Buffer.alloc() based on their
 * specific needs. There is no hard deprecation because of the extent to
 * which the Buffer constructor is used in the ecosystem currently -- a
 * hard deprecation would introduce too much breakage at this time.
 * Eventually, however, we should re-evaluate that choice.
 **/
...
Buffer.alloc = function(size, fill, encoding) {
  ...
  if (fill !== undefined) {
    ...
    if (typeof encoding === 'string') {
      ...
      if (size > 0)
        flags[kNoZeroFill] = 1;
      return createBuffer(size).fill(fill, encoding);
    } else {
      // Since we are filling anyway, don't zero fill initially
      if (size > 0)
        flags[kNoZeroFill] = 1;
...
Buffer.allocUnsafe = function(size) {
  if (arguments.length !== 1)
    throw new TypeError(`allocUnsafe(size) requires only a single argument`);
  if (typeof size !== 'number')
    throw new TypeError(`'size' argument must be a number`);
  size = Math.max(0, size);
  if (size > 0)
