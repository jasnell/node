...
function Buffer(arg, encoding) {
  ...
  if (typeof arg === 'number') {
    deprecationWarned = deprecated(
      'Buffer(number) is deprecated. Please use Buffer.zalloc() to allocate ' +
      'a new zero-filled Buffer, or Buffer.alloc() to allocate a new ' +
...
Buffer.zalloc = function(num) {
  ...
  // Buffer.zalloc is called, which is quite expensive for small buffer sizes.
...
function SlowBuffer(length) {
  sbdeprecationWarned = deprecated(
    'SlowBuffer(number) is deprecated. Please use SlowBuffer.zalloc() to ' +
    'allocate a new zero-filled SlowBuffer, or SlowBuffer.alloc() to' +
    'allocate a new uninitialized SlowBuffer.', sbdeprecationWarned);
  return SlowBuffer.alloc(length);
}
 ...
 **/
SlowBuffer.zalloc = function(num) {
  if (+num != num)
    num = 0;
  flags[kNoZeroFill] = 0;
