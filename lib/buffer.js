...
const bindingObj = {};
const deprecated = require('internal/util').printDeprecationMessage;
...
function Buffer(arg, encoding) {
  ...
  if (typeof arg === 'number') {
    deprecationWarned = deprecated(
      'Buffer(number) is deprecated. Please use Buffer.safe() to allocate ' +
      'a new zero-filled Buffer, or Buffer.unsafe() to allocate a new ' +
      'uninitialized Buffer.', deprecationWarned);
    if (typeof encoding === 'string') {
      throw new Error(
        'If encoding is specified then the first argument must be a string'
      );
    }
    return Buffer.unsafe(arg);
  }
...
/**
 * Creates a new zero-filled Buffer instance.
 **/
Buffer.safe = function(num) {
  // benchmarks currently indicate that it's generally faster to allocate
  // first from the slab then fill even for relatively large Buffer sizes.
  // The alternative is to drop straight down to createBuffer() with
  // flags[kNoZeroFill] = 0. Doing so would force a calloc every time
  // Buffer.safe is called, which is quite expensive for small buffer sizes.
  // Later on, we may want to revisit this if we can identify a more efficient
  // way of allocating zero filled Buffers.
  const buf = Buffer.unsafe(num);
  // if process.zeroFillAllBuffers is true, buffer will already be filled
  if (!process.zeroFillAllBuffers) buf.fill(0);
  return buf;
};

/**
 * Equivalent to Buffer(num), creates a non-zero-filled Buffer instance.
 **/
Buffer.unsafe = function(num) {
  if (typeof num !== 'number')
    throw new TypeError(`'num' argument must be a number`);
  num = Math.max(0, num) || 0;
  if (num > 0)
    flags[kNoZeroFill] = 1;
  const buf = allocate(num);
  // if process.zeroFillAllBuffers is true, we must fill the buffer
  if (process.zeroFillAllBuffers) buf.fill(0);
  return buf;
};

function SlowBuffer(length) {
  sbdeprecationWarned = deprecated(
    'SlowBuffer(number) is deprecated. Please use SlowBuffer.safe() to ' +
    'allocate a new zero-filled SlowBuffer, or SlowBuffer.unsafe() to' +
    'allocate a new uninitialized SlowBuffer.', sbdeprecationWarned);
  return SlowBuffer.unsafe(length);
}
...
/**
 * Creates a new zero-filled Buffer instance.
 * Because SlowBuffer always falls directly back to createBuffer,
 * we simply set flags[kNoZeroFill] = 0 so calloc is called to
 * allocate the buffer instead of malloc. It's *really slow*, but
 * it's better than calling fill(0) after.
 **/
SlowBuffer.safe = function(num) {
  if (+num != num)
    num = 0;
  flags[kNoZeroFill] = 0;
  const buf = createBuffer(+num);
  flags[kNoZeroFill] = 1;
  return buf;
};

/**
 * Creates a new non-zero-filled SlowBuffer
 **/
SlowBuffer.unsafe = function(num) {
  // If process.zeroFillAllBuffers, the buffer must be zero-filled
  if (process.zeroFillAllBuffers)
    return SlowBuffer.safe(num);
  if (+num != num)
    num = 0;
  if (num > 0)
    flags[kNoZeroFill] = 1;
...
Buffer.concat = function(list, length) {
  ...
  var buffer = Buffer.unsafe(length);
  var pos = 0;
  for (var i = 0; i < list.length; i++) {
    var buf = list[i];
    buf.copy(buffer, pos);
