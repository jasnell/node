...
const policyBinding = internalBinding('policy');
const policyBindingBuf = new Uint8Array(policyBinding.detail);
...
const policy = {
  granted(permission) {
    ...
    policyBinding.getCurrent();
    const index = policyBinding.nameMap[permission];
    return index !== undefined ? (policyBindingBuf[index] === 1) : false;
  },
  ...
  deny(permissions) {
    ...
    policyBinding.deny(permissions);
  }
};
