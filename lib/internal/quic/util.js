...
const {
  ...
  constants: {
    ...
    IDX_QUICSOCKET_CONFIG_RETRY_TOKEN_EXPIRATION,
    IDX_QUICSOCKET_CONFIG_MAX_CONNECTIONS,
    IDX_QUICSOCKET_CONFIG_MAX_CONNECTIONS_PER_HOST,
    IDX_QUICSOCKET_CONFIG_MAX_STATELESS_RESETS_PER_HOST,
    IDX_QUICSOCKET_CONFIG_OPTIONS,
    IDX_QUICSOCKET_CONFIG_DISABLE_STATELESS_RESET,
    IDX_QUICSOCKET_CONFIG_QLOG,
...
const socketConfigView = new DataView(socketConfig);
...
function validateQuicStreamOptionsH3(options) {
  const {
...
function validateQuicStreamOptionsDefault(options) {
  const {
    unidirectional = false,
    body,
  } = options;

  validateBoolean(unidirectional, 'options.undirectional');

  // TODO(@jasnell): Validate body options
...
function validateQuicStreamOptions(alpn, options = {}) {
  validateObject(options, 'options');
  switch (alpn) {
    case 'h3-29':
      return validateQuicStreamOptionsH3(options);
    default:
      return validateQuicStreamOptionsDefault(options);
  }
}
...
function setQuicSocketConfig(options = {}) {
  const {
    retryTokenTimeout,
    maxConnections,
    maxConnectionsPerHost,
  ...
  socketConfigView.setBigInt64(
    IDX_QUICSOCKET_CONFIG_RETRY_TOKEN_EXPIRATION,
    BigInt(retryTokenTimeout));

  socketConfigView.setBigInt64(
    IDX_QUICSOCKET_CONFIG_MAX_CONNECTIONS,
    BigInt(maxConnections));

  socketConfigView.setBigInt64(
    IDX_QUICSOCKET_CONFIG_MAX_CONNECTIONS_PER_HOST,
    BigInt(maxConnectionsPerHost));

  socketConfigView.setBigInt64(
    IDX_QUICSOCKET_CONFIG_MAX_STATELESS_RESETS_PER_HOST,
    BigInt(maxStatelessResetsPerHost));

  socketConfigView.setUint32(
    IDX_QUICSOCKET_CONFIG_OPTIONS,
    socketOptions);

  socketConfigView.setUint8(
    IDX_QUICSOCKET_CONFIG_DISABLE_STATELESS_RESET,
    disableStatelessReset ? 1 : 0);
