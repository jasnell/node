...
class EndpointConfig extends JSTransferable {
  constructor(options = {}) {
    ...
    validateString(address, 'config.address');
    validatePort(port, 'config.port', { allowZero: true });
  ...
  [kDeserialize]({ handle, options, type }) {
    this[kHandle] = handle;
    this[kOptions] = options;
  }

  [kUdp](udp = 'udp4') {
    if (typeof udp === 'string') {
      this[kOptions].udp = { type: 'udp4' };
    } else if (udp != null && typeof udp === 'object') {
      const {
        type = 'udp4',
        reuseAddr = false,
        ipv6Only = false,
      ...
      validateString(type, 'config.udp.type');
      validateBoolean(reuseAddr, 'config.udp.reuseAddr');
      validateBoolean(ipv6Only, 'config.udp.ipv6Only');
      if (recvBufferSize !== undefined)
        validateNumber(recvBufferSize, 'config.udp.recvBufferSize');
      if (sendBufferSize !== undefined)
        validateNumber(sendBufferSize, 'config.udp.sendBufferSize');
      this[kOptions].udp = {
        type,
        reuseAddr,
        ipv6Only,
      ...
      throw new ERR_INVALID_ARG_TYPE('config.udp', ['string', 'object'], udp);
    }
  }
}
...
class SessionOptions extends JSTransferable {
  ...
  [kSetPreferredAddress](addr, name, family) {
    ...
    validatePort(port, `${name}.port`, { allowZero: false });
