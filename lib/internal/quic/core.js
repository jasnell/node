...
function onStreamReady(handle) {
  const session = this[owner_symbol];
  const stream = QuicStream[kSyncCreate](
    session,
    handle,
    undefined,
    QuicInboundStream[kSyncCreate]());
...
class QuicSession extends EventEmitter {
  ...
  // Closes the specified stream with the given code and removes it
  // from the collection of tracked streams.
  [kStreamClose](id, code) {
    const { streams } = this[kInternalState];
    const stream = streams.get(id);
    if (stream !== undefined) {
      stream[kDestroy](code);
      streams.delete(stream);
    }
    // In case close() has been called on the session, calling
    // [kMaybeDestroy] here may trigger the completion of the
    // close operation if there are no additional open streams
    // remaining.
    this[kMaybeDestroy]();
  }
  ...
  // Returns a promise that creates a new stream. The options
  // supported will depend on the application protocol selected.
  // For default QUIC applications (that is, non-HTTP3), the
  // options are undirectional and body. If the stream is to
  // have outbound data, body must be provided, otherwise the
  // writable side of the stream will be closed immediately
  // after creation.
  //
  // e.g.
  // const stream = await session.createStream({ body: 'hello world' })
  ...
  async createStream(options) {
    ...
    const {
      unidirectional,
      body: source,
    } = validateQuicStreamOptions(this.alpn, options);

    // TODO(@jasnell): For HTTP/3 and other applications that support
    // headers... informational, initial, and trailing headers will
    // be provided in the options either as an object, map, or promise
    // resolving an object or a map.
...
class HeadersEvent extends Event {
  constructor(stream, kind, headers) {
    super('headers');
    this[kKind] = headerKindName(kind);
    this[kHandle] = stream;
  ...
  // "informational", "initial", or "trailing"
  get kind() { return this[kKind]; }

  get stream() { return this[kHandle]; }
}
...
class QuicStream extends EventTarget {
  ...
  static [kSyncCreate](session, handle, source, stream) {
    if (stream === undefined) {
      stream = new EventTarget();
      ObjectSetPrototypeOf(stream, QuicStream.prototype);
    }

    defineEventHandler(stream, 'close');
    defineEventHandler(stream, 'error');
    defineEventHandler(stream, 'headers');

    const id = handle.id();
    const unidirectional = Boolean(id & 0b10);
    const serverInitiated = Boolean(id & 0b01);
    ...
    stream[kInternalState] = {
      ...
      closePromiseReject: undefined,
      closePromiseResolve: undefined,
      dataRateHistogram: new Histogram(handle.rate),
      dataSizeHistogram: new Histogram(handle.size),
      dataAckHistogram: new Histogram(handle.ack),
      resetCode: undefined,
      session: session,
      sharedState: new QuicStreamSharedState(handle.state),
      source: source
    };

    stream[kHandle] = handle;
    stream[async_id_symbol] = handle.getAsyncId();
    handle[owner_symbol] = stream;
    session[kAddStream](id, stream);

    return stream;
  }

  // The asynchronous factory function waits in parallel for both the
  // completion of the TLS handshake and the acquisition of the outbound
  // payload source then proceeds to create the QuicStream object. This
  // is an internal only function that is called by the createStream
  // function.
  static async [kAsyncCreate](session, options) {
    ...
    return QuicStream[kSyncCreate](
      session,
      _createStream(session[kHandle], ret[1], unidirectional),
      ret[1],
      undefined);
  }
  ...
  // The [kClose] method is called when the owning session
  // receives notification that the underlying stream has
  // been closed. At this point, the link to the session
  // is severed and all other persistent refs to the stream
  // object have been freed. The QuicStream object is free
  // to be garbage collected unless user code has retained
  ...
  [kHeaders](headers, kind) {
    this.dispatchEvent(new HeadersEvent(this, kind, headers));
  }
  ...
  [kClose]() {
    ...
    // TODO(@jasnell): Mark the source closed on
    // the JavaScript side.

    // Mark the handle closed.
    this[kHandle].close();

    // Create and return the deferred close promise
    state.closePromise = promise;
    state.closePromiseReject = reject;
    state.closePromiseResolve = resolve;
    return PromisePrototypeFinally(promise, () => {
  ...
  get detached() {
    return this[kInternalState].session === undefined;
  }

  get serverInitiated() {
    return this[kInternalState].serverInitiated;
  }

  get unidirectional() {
    return this[kInternalState].unidirectional;
  }

  get resetCode() {
    return this[kInternalState].resetCode;
  }

  get closeCode() {
    return this[kInternalState].closeCode;
  }
...
// A peer-initiated inbound stream provides additional
// methods that are not available on locally-initiated
// streams, such as send() to attach a data source,
// and sendHeaders options...
class QuicInboundStream extends QuicStream {
  static [kSyncCreate]() {
    const stream = new EventTarget();
    ObjectSetPrototypeOf(stream, QuicInboundStream.prototype);
    return stream;
  }

  // Instances of QuicInboundStream cannot be created directly.
  constructor() { throw new ERR_ILLEGAL_CONSTRUCTOR(); }
  ...
  async send(source, options) {
    ...
    // TODO(@jasnell): Prevents send from being called again.
    state.source = source;
    ...
    // TODO(@jasnell): This should return a Promise that resolves when the
