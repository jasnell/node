...
async function acquireSource(data, options) {
  ...
  if (typeof data === 'string') {
    ...
    return new ArrayBufferViewSource(handle, Buffer.from(data, encoding));
  }

  if (isArrayBufferView(data)) {
    return new ArrayBufferViewSource(handle, data);
  }
...
class QuicStream extends EventEmitter {
  ...
  static async [kAsyncCreate](session, options) {
    ...
    return QuicStream[kSyncCreate](
      session,
      unidirectional ?
        _openUnidirectionalStream(session[kHandle], ret[1]) :
        _openBidirectionalStream(session[kHandle], ret[1]),
      ret[1]);
  }
  ...
  [kDestroy](code) {
    this[kSetHandle]();
    const state = this[kInternalState];
    // TODO(@jasnell): What cleanup on the source and consumer is needed?
    // TODO(@jasnell): What to do with the code?
  }
