...
class QuicSession extends EventEmitter {
  ...
  async openStream(options) {
    ...
    const {
      unidirectional,  // Unidirectional or Bidirectional
      highWaterMark,
      defaultEncoding,
...
class QuicStream extends Duplex {
  ...
  constructor(options, session, handle) {
    const {
      highWaterMark,
      defaultEncoding,
      readable = true,
  ...
  [kInspect](depth, options) {
    const direction = this.unidirectional ? 'unidirectional' : 'bidirectional';
