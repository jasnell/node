...
class Session {
  ...
  open(options = new StreamOptions()) {
    ...
    if (!(options instanceof StreamOptions))
...
class Endpoint {
  ...
  async connect(origin, options = new SessionOptions()) {
    ...
    const handle = this[kHandle]?.createClientSession(
    ...
    const clientSession = internalSession(handle);
    this[kState].get('sessions').add(clientSession);
  ...
  listen(options = new SessionOptions()) {
    ...
    if (!(options instanceof SessionOptions))
      options = new SessionOptions(options);
    const context = options[kCreateServerContext]();
    this[kHandle].listen(options[kHandle], context);
  ...
  [kOnSessionNew](session) {
    const serverSession = internalSession(session);
...
function internalSession(handle) {
  return ReflectConstruct(
    function (handle) { this[kHandle] = handle; },
    [ handle ],
    Session
  );
}

function internalStream(handle) {
  return ReflectConstruct(
    function (handle) { this[kHandle] = handle; },
    [ handle ],
    Stream);
