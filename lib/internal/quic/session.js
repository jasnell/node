...
ObjectDefineProperties(OCSPRequest.prototype, {
  certificate: { enumerable: true },
  issuer: { enumerable: true },
  respondWith: { enumerable: true },
});
...
ObjectDefineProperties(ClientHello.prototype, {
  alpn: { enumerable: true },
  ciphers: { enumerable: true },
  servername: { enumerable: true },
...
class Session extends NodeEventTarget {
  ...
  [kMaybeStreamEvent](stream) {
    if (this[kState].alpn === HTTP3_ALPN &&
        stream[kState].headers === undefined) {
      return;
    }
    PromisePrototypeThen(
      PromiseResolve(),
  ...
  [kSessionTicket](blob) {
    this[kState].sessionTicket = blob;
  }
  ...
  open(options = new StreamOptions()) {
    ...
    PromisePrototypeThen(
      acquireBody(body),
      (actual) => setStreamSource(stream, actual),
...
ObjectDefineProperties(Session.prototype, {
  ...
  certificate: { enumerable: true },
  cipher: { enumerable: true },
  clienthello: { enumerable: true },
  close: { enumerable: true },
  closed: { enumerable: true },
  closing: { enumerable: true },
  earlyData: { enumerable: true },
  ephemeralKeyInfo: { enumerable: true },
  handshake: { enumerable: true },
  keylog: { enumerable: true },
  ocsp: { enumerable: true },
  open: { enumerable: true },
  peerCertificate: { enumerable: true },
  qlog: { enumerable: true },
  remoteAddress: { enumerable: true },
  servername: { enumerable: true },
  stats: { enumerable: true },
  updateKey: { enumerable: true },
  validationError: { enumerable: true },
});
    ...
    class extends NodeEventTarget {
      constructor() {
        ...
        this[kState] = {
          ...
          inner,
          keylog: handle.keylog ? createLogStream(handle.keylog) : undefined,
          maxPacketLength: 0,
          ocsp: undefined,
          qlog: handle.qlog ? createLogStream(handle.qlog) : undefined,
...
function sessionRemoteAddress(session) {
  return session[kHandle]?.getRemoteAddress();
}

function sessionPeerCertificate(session) {
  return session[kHandle]?.getPeerCertificate();
}

function sessionCertificate(session) {
  return session[kHandle]?.getCertificate();
}
